    ...
    <MainLayout>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {dummyScans.map((scan) => (
            <Card key={scan.id} className="rounded-2xl shadow-md overflow-hidden flex flex-col group transition-transform duration-300 hover:-translate-y-2">
              <CardHeader className="p-0 relative">
                <img src={scan.imageUrl} alt={scan.label} className="w-full h-48 object-cover" />
                <div className="absolute top-4 right-4">
                  <Badge className={cn("text-sm", getBadgeStyle(scan.label))}>{scan.label}</Badge>
                </div>
              </CardHeader>
              <CardContent className="p-6 flex-1 flex flex-col">
                <div className="flex-1">
                  <p className="text-sm text-muted-foreground">Scanned on: {scan.date}</p>
                  <div className="font-bold text-2xl mt-1">{Math.round(scan.confidence * 100)}% Confidence</div>
                </div>
                <Button variant="outline" className="w-full mt-4">View Certificate</Button>
              </CardContent>
            </Card>
          ))}
        </div>
