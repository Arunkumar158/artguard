...
export default function ScanHistory() {
  ...
  const handleSearch = async (query: string) => {
    ...
    if (query.trim()) {
      try {
        ...
        toast({
          title: "Search Error",
          description: "Failed to search scans.",
          variant: "destructive",
        });
      }
    } else {
  ...
  const getImageUrl = (scan: ScanHistoryRecord): string => {
    // Priority: Cloudinary URL > image_url > artwork_url > upload_url
    return scan.result.cloudinary_url || 
           scan.image_url || 
           scan.artwork_url || 
           scan.upload_url || 
           scan.result.upload_url || 
           "";
  };
