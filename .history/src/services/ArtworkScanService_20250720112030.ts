...
class ArtworkScanService {
  ...
  static async completeScanProcess(
    ...
    try {
      ...
      // Use the backend API to log to Supabase
      const formData = new FormData();
      ...
      const logResponse = await fetch(`${API_BASE_URL}/upload/`, {
        method: 'POST',
        body: formData,
      });

      if (!logResponse.ok) {
        console.warn('Failed to log scan to Supabase, but scan completed successfully');
      }
